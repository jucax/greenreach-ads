...
try {
  ...
  // Test 7: Check users table schema
  console.log('\n7️⃣ Checking users table schema...');
  try {
    const { data, error } = await supabase
      .from('users')
      .select('*')
      .limit(0); // Just get schema info
    
    if (error) {
      console.error('❌ Users table schema error:', error);
    } else {
      console.log('✅ Users table schema accessible');
    }
  } catch (err) {
    console.error('❌ Users table schema failed:', err);
  }
...
console.log('\n🏁 Registration test complete! Check the results above.');
